...
import re
import datetime
from shapely.geometry import LineString # calculates intersection between finish line and current race route
...
class Sensors:
    ...
    start_gps_cords = () 
    gps_index = 0
    # start_gps_cords = {'lat': 0, 'long': 0}
    # now_gps_cords = {'lat': 0, 'long': 0}
    # finish_gps_cords = {
    #     'lat_part_a': 0,
    #     'long_part_a':0,
    #     'lat_part_b': 0,
    ...
    def gps_distance(self, gps_present_latitude, gps_present_longitude, gps_past_latitude, gps_past_longitude):
        ...
        gps_difference = geopy.distance.distance(gps_past, gps_present).miles
    ...
    def speed_calc(self, TIRE_RADIUS_REF, TIRE_PRESSURE_REF):
        ...
        if Sensors.switch_mph_reference == False:   
            ...
            ####################################################################################################
            # # IN DEVELOPMENT NEW CODE FOR GPS 
            Sensors.mph = round(GPS.speed_in_mph, 2) # read mph from GPS module 
            # gps reference is activated 
            if Sensors.race_started == True: # race started 
                print("race started ")
                # wait 1 second to start collecting data 
                if time.time() - Sensors.last_gps_record >= 1:  
                    print("race started 2")

                    # refresh last gps recorded 
                    Sensors.last_gps_record = time.time()

                    # collect first and current gps data into a record list
                    Sensors.record_route.append((GPS.latitude, GPS.longitude))

                    # if record list is bigger enough to compare their data 
                    if len(Sensors.record_route) > 1:
                        
                        # if index has reached the max 
                        if Sensors.gps_index >= len(Sensors.record_route):
                            print("race ended")
                            # TODO: SET FLAG TO END THE RACE 
                            Sensors.race_started = False    # let's try by clearing this 
                        
                        # calculate distance traveled 
                        Sensors.gps_dist_travel = Sensors.gps_dist_travel + self.gps_distance(
                            Sensors.record_route[Sensors.gps_index][0],  # current latitude
                            Sensors.record_route[Sensors.gps_index][1], # current longitude 
                            Sensors.record_route[Sensors.gps_index - 1][0],  # last latitude 
                            Sensors.record_route[Sensors.gps_index - 1][1]) # lat longitude 
                        print("distance_Traveled: {}".format(Sensors.gps_dist_travel))
                        # calculate intersection
                        # finish line cords [point a, pointb]
                        # point a or point b format: (lat, long)
                        finish_line = {
                            'point_a': (11.010726, -74.905604),
                            'point_b': (11.011456, -74.899899)
                        }
                        line1 = LineString([finish_line["point_a"], finish_line["point_b"]])
                        line2 = LineString(Sensors.record_route)

                    # if len(Sensors.record_route) < 1:
                    #     Sensors.record_route.append(Sensors.start_gps_cords)

                    # else:
                    #     # collect data into record list 
                    #     Sensors.record_route.append((GPS.latitude, GPS.longitude))
                    # if Sensors.first_gps_record:
                    #     Sensors.record_route.append(Sensors.start_gps_cords) 
                    #     Sensors.first_gps_record = False 

                    # else:
                    #     # collect data into record list 
                    #     Sensors.record_route.append(dict(lat=GPS.latitude, long=GPS.longitude))

                    #     # calculate current distance traveled 
                    #     Sensors.gps_dist_travel = Sensors.gps_dist_travel + \
                    #         self.gps_distance(
                    #             Sensors.record_route[Sensors.record_index]['lat'],  # current latitude
                    #             Sensors.record_route[Sensors.record_index]['long'], # current longitude 
                    #             Sensors.record_route[Sensors.record_index - 1]['lat'],  # last latitude 
                    #             Sensors.record_route[Sensors.record_index - 1]['long']) # lat longitude 

                    #     record_index += 1

                        #calculate intersection between 

                    # calculate/measure distance 
                    # gps_dist_travel.
                    # if 
                ...
                Sensors.last_gps_record = time.time()
                Sensors.start_gps_cords = (GPS.latitude, GPS.longitude)
                # Sensors.start_gps_cords['lat'] = GPS.latitude
                # Sensors.start_gps_cords['long'] = GPS.latitude
            ####################################################################################################
            # OLD CODE FROM GPS 
            # if Sensors.race_started == True: # race started 
